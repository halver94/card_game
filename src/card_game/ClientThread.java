...
class ClientThread implements Runnable
{
	...
	private Server server; // pour utilisation des méthodes de la classe principale
	...
	ClientThread(Socket s, Server server) // le param s est donnée dans BlablaServ par ss.accept()
	{
		this.server = server; // passage de local en global (pour gestion dans les autres méthodes)
		this._s = s; // passage de local en global
		try
		{
			...
			_numClient = server.addClient(_out);
		}
		catch (IOException e){ }
			...
			while(_in.read(charCur, 0, 1)!=-1) // attente en boucle des messages provenant du client (bloquant sur _in.read())
			{
				...
				else if(!message.equalsIgnoreCase("")) // juste une vérification de principe
				{
					if(charCur[0]=='\u0000') {// le dernier caractère était '\u0000' (char de terminaison nulle)
						...
						server.treat(String.valueOf(_numClient) , message + charCur[0]);
					}
					else {
						server.treat(String.valueOf(_numClient) , message);
